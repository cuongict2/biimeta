...
function CustomerOrderContent() {
  ...
  useEffect(() => {
    ...
    if (savedNickname) {
      ...
      const defaultNickname = `${randomAnimal} ${randomColor} #${randomNum}`;
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    setChatInput("");
    setShowIconPicker(false); // <--- THÊM DÒNG NÀY
    ...
    <div style={currentStyles.page}>
      ...
      {activeTab === "chat" && !orderCreated && (
        <div style={{ ...currentStyles.glass, padding: "20px", display: "flex", flexDirection: "column", height: "calc(100vh - 160px)", minHeight: "450px" }}>
                {/* Header Tab Chat - Cách đơn giản nhất */}
            ...
            {isChangingNickname ? (
              <div style={{ display: "flex", gap: "8px", alignItems: 'center' }}>
                ...
                <button 
                  ...
                  style={{ background: "#10b981", color: "#fff", border: "none", borderRadius: "8px", padding: "8px 12px", cursor: "pointer" }}
                >
                  Lưu
                </button>
              </div>
...
{showIconPicker && (
  <div style={{ 
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(44px, 1fr))", gap: "8px" }}>
      {/* 1. Render Emoji */}
      {["❤️", "😂", "👍", "🙏", "🎉", "😮", "😍", "😢", "🤔", "👏"].map(emoji => (
        <button key={emoji} type="button" onClick={() => setChatInput(prev => prev + emoji)} style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", height: "44px", width: "44px", cursor: "pointer", fontSize: "1.3rem", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {emoji}
        </button>
      ))}
      
      {/* 2. Render Yahoo Icons cùng style */}
      {["bawling.gif", "bow.gif", "fearful.gif", "nuh_uh.gif", "pensive.gif", "relaxed.gif", "rofl.gif", "smiley.gif", "wink.gif", "worried.gif"].map(icon => (
        <button key={icon} type="button" onClick={() => setChatInput(prev => prev + `[YAHOO:${icon}]`)} style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", height: "44px", width: "44px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <img src={`/yahoo_icons/${icon}`} alt={icon} style={{ width: "28px", height: "28px" }} />
        </button>
      ))}
    </div>
  </div>
  ...
  display: "flex", 
  gap: "6px", // Giảm gap một chút cho màn hình nhỏ
  alignItems: "center",
  width: "100%",
  boxSizing: "border-box" 
}}>
  
  {/* Nút bật/tắt bảng chọn Icon - Thêm flexShrink: 0 */}
  <button
    type="button"
    onClick={() => setShowIconPicker(prev => !prev)}
    style={{
      width: "44px", height: "44px", borderRadius: "12px",
      background: showIconPicker ? "rgba(59, 130, 246, 0.2)" : "rgba(255,255,255,0.06)",
      border: `1px solid ${showIconPicker ? "rgba(59, 130, 246, 0.4)" : "rgba(255,255,255,0.1)"}`,
      display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", 
      fontSize: '1.4rem',
      flexShrink: 0 // <--- CỐ ĐỊNH NÚT, KHÔNG ĐỂ BỊ TRÀN
    }}
  >
    🙂
  </button>
  ...
  <input 
    ...
    style={{ 
      ...
      minWidth: "50px", // Đảm bảo input không biến mất
  ...
  <button type="submit" disabled={!chatInput.trim() || isUploadingImage} style={{ 
    ...
    {isUploadingImage ? "⏳" : "🚀"}
      ...
      <style>{`
        ...
        .message-entry {
          animation: fadeInSlide 0.4s cubic-bezier(0.22, 1, 0.36, 1) forwards;
        }
      `}</style>
